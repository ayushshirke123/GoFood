...
export default function Cart() {
  ...
  const handleCheckOut = async () => {
    ...
    let response = await fetch(`${getApiBaseUrl()}/api/auth/orderData`, {
      // credentials: 'include',
          ...
          <tbody className='text-white'>
            {data.map((food, index) => (
              <tr key={index} className='text-white'>
                ...
                <td className='text-white'><button type="button" className="btn p-0 text-white"><DeleteIcon onClick={() => { dispatch({ type: "REMOVE", index: index }) }} /></button> </td></tr>
            ))}
          </tbody>
        </table>
